...
func newSQLWithDB(ctx context.Context, db *sqlx.DB, prefix string, options *Options) (*SQL, error) {
	...
	store := &SQL{db: db, opts: options, table: table, quoted: table.Quoted(), members: members}
...
func (store *SQL) checkSchema(ctx context.Context) error {
	...
	want := map[string]storage.SQLColumn{
		...
		"value_kind":           {Type: "BIGINT", Nullable: false},
...
func (store *SQL) getUnlocked(ctx context.Context, key Key) ([]byte, error) {
	...
	var kind int
	query := store.db.Rebind("SELECT value, expires_at_unix_nano, value_kind FROM " + store.quoted + " WHERE encoded_key = ?")
	err := store.db.QueryRowContext(ctx, query, encoded).Scan(&value, &expires, &kind)
...
func (store *SQL) BatchMutate(ctx context.Context, entries []Entry, keys []Key) error {
	...
	if err := store.clearMembers(ctx, tx, mutationKeys); err != nil {
		return err
	}
...
func (store *SQL) CompareAndMutate(ctx context.Context, guard Key, expected []byte, entries []Entry, keys []Key) (bool, error) {
	...
	if err := store.clearMembers(ctx, tx, store.mutationKeys(entries, keys)); err != nil {
		return false, err
	}
	for _, entry := range prepared {
		if err := store.upsert(ctx, tx, entry); err != nil {
...
func (store *SQL) upsert(ctx context.Context, tx *sqlx.Tx, entry sqlEntry) error {
	query := store.db.Rebind("INSERT INTO " + store.quoted + " (encoded_key, value, expires_at_unix_nano) VALUES (?, ?, ?) ON CONFLICT(encoded_key) DO UPDATE SET value = excluded.value, value_kind = 0, expires_at_unix_nano = excluded.expires_at_unix_nano")
