...
func TestOpenBackendReopensCurrentRetrievalIndex(t *testing.T) {
	backend := freshBackend(t)
	ctx := t.Context()
	doc := retrieval.Doc{ID: "current", Content: "likes tea", Metadata: map[string]any{"kind": "preference", "confidence": 0.9}, Timestamp: time.Now()}
	if err := backend.RetrievalIndex().Upsert(ctx, "workspace", []retrieval.Doc{doc}); err != nil {
		t.Fatal(err)
	}
	reopened, err := OpenBackend(ctx, backend.client, backend.prefix)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = reopened.Close() })
	response, err := reopened.RetrievalIndex().Search(ctx, "workspace", retrieval.SearchRequest{
		QueryText: "tea",
		Filter: retrieval.Filter{
			Eq:    map[string]any{"kind": "preference"},
	...
	if len(response.Hits) != 1 || response.Hits[0].Doc.ID != doc.ID {
		t.Fatalf("Search() hits = %#v, want retained document", response.Hits)
	}
}
