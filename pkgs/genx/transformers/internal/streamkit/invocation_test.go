...
func TestInvocationInterruptKeepsPulledPrefixAndDiscardsBufferedSuffix(t *testing.T) {
	...
	response, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain", "audio/opus")
	if err != nil {
	...
	if err := invocation.Interrupt(response, ""); err != nil {
	...
	replacement, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain")
	if err != nil {
...
func TestInvocationInterruptReplacesDiscardedRouteEOS(t *testing.T) {
	...
	response, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain", "audio/pcm")
	if err != nil {
		t.Fatalf("StartResponse() error = %v", err)
	}
	if err := invocation.Emit(response, &genx.MessageChunk{Role: genx.RoleUser, Part: genx.Text("visible")}); err != nil {
...
func TestInvocationCancellationDoesNotCrossInvocationBoundary(t *testing.T) {
	...
	defer first.Close()
	defer second.Close()
	firstResponse, _ := first.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain")
	secondResponse, _ := second.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain")
...
func TestInvocationParentCancellationClosesOutput(t *testing.T) {
	...
	defer invocation.Close()
	response, _ := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain")
	cancel()
	chunk, err := invocation.Output().Next()
	if err != nil {
		t.Fatalf("Next() error = %v", err)
...
func TestInvocationStartsClosedForCancelledParent(t *testing.T) {
	...
	cancel()
	invocation := NewInvocation(ctx, OutputConfig{})
	if _, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain"); !errors.Is(err, io.ErrClosedPipe) {
...
func TestInvocationOutputCloseCancelsInvocation(t *testing.T) {
	invocation := NewInvocation(context.Background(), OutputConfig{})
	response, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain")
	if err != nil {
		t.Fatalf("StartResponse() error = %v", err)
	}
...
func TestInvocationSupportsInterleavedResponses(t *testing.T) {
	invocation := NewInvocation(context.Background(), OutputConfig{})
	defer invocation.Close()
	response, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "text/plain")
	if err != nil {
	...
	second, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "transcript"}, "audio/opus")
	if err != nil {
		t.Fatalf("second StartResponse() error = %v", err)
	}
	if second.StreamID() == response.StreamID() {
		t.Fatalf("interleaved responses shared StreamID %q", response.StreamID())
	}
	if err := invocation.Emit(response, &genx.MessageChunk{
		Role: genx.RoleUser,
	...
	if err := invocation.Emit(response, &genx.MessageChunk{Role: genx.RoleUser, Part: genx.Text("ok")}); err != nil {
		t.Fatalf("Emit() error = %v", err)
	}
	...
	if _, err := invocation.StartResponse(ResponseConfig{Role: genx.RoleUser, Label: "source"}, "audio/opus"); err != nil {
		t.Fatalf("replacement StartResponse() error = %v", err)
	}
}

func TestInvocationRejectsDuplicateActiveStreamID(t *testing.T) {
	invocation := NewInvocation(context.Background(), OutputConfig{})
	defer invocation.Close()
	first, err := invocation.StartResponse(ResponseConfig{StreamID: "shared"}, "text/plain")
	if err != nil {
		t.Fatalf("first StartResponse() error = %v", err)
	}
	if _, err := invocation.StartResponse(ResponseConfig{StreamID: "shared"}, "audio/pcm"); !errors.Is(err, ErrResponseActive) {
		t.Fatalf("duplicate StartResponse() error = %v, want ErrResponseActive", err)
	}
	if err := invocation.FinishResponse(first, ""); err != nil {
		t.Fatalf("FinishResponse() error = %v", err)
	}
	if _, err := invocation.StartResponse(ResponseConfig{StreamID: "shared"}, "audio/pcm"); err != nil {
		t.Fatalf("reused completed StreamID error = %v", err)
	}
}
