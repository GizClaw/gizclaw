...
func createWorkflow(ctx context.Context, c *gizcli.Client, doc apitypes.Workflow) (apitypes.Workflow, error) {
	api, err := c.ServerAdminClient()
	if err != nil {
		return apitypes.Workflow{}, err
	}
	resp, err := api.CreateWorkflowWithResponse(ctx, adminhttp.WorkflowUpsert{Name: doc.Name, Spec: doc.Spec})
	if err != nil {
		return apitypes.Workflow{}, err
	}
	if resp.JSON200 != nil {
...
func putWorkflow(ctx context.Context, c *gizcli.Client, name string, doc apitypes.Workflow) (apitypes.Workflow, error) {
	api, err := c.ServerAdminClient()
	if err != nil {
		return apitypes.Workflow{}, err
	}
	resp, err := api.PutWorkflowWithResponse(ctx, name, adminhttp.WorkflowUpsert{Name: doc.Name, Spec: doc.Spec})
	if err != nil {
		return apitypes.Workflow{}, err
	}
	if resp.JSON200 != nil {
