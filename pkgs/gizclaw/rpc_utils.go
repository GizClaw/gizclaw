...
func rpcObservationResult(canceled bool, code int, err error) observability.Result {
	...
	// Codes are canonical status codes, so the server/client split comes from
	// the HTTP status they project onto, not from an HTTP range check on the
	// code itself.
	if rpcapi.StatusCode(code).HTTPStatus() >= 500 {
