...
func (s *Server) login(ctx context.Context, request peerhttp.LoginRequestObject, authorizer SessionAuthorizer) (peerhttp.LoginResponseObject, error) {
	...
	case request.Body == nil || isBodylessLogin(ctx):
		...
		if registration != nil {
			commitOwnerProfile = func(commit func() error) error {
				...
				if err := s.OwnerProfileBinder(ctx, publicKey.String(), registration.RuntimeProfile.Id, commit); err != nil {
	...
	if registration != nil {
		slog.InfoContext(ctx, "public HTTP registration accepted", "peer_public_key", publicKey.String(), "registration_token", registration.TokenName, "runtime_profile", registration.RuntimeProfile.Id)
	}
