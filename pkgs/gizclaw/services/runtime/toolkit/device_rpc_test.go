...
func TestExecutorRegistryDispatchesDeviceAndBuilderTracksReconnect(t *testing.T) {
	...
	request := BuildRequest{CallerPublicKey: "peer-a", ProfileToolIDs: []string{"peer.peer-a.music.play"}}
	kit, err := builder.Build(ctx, request)
	...
	if _, err := builder.Invoke(ctx, registry, InvokeRequest{Build: request, Name: kit.Tools[0].ID}); err != nil {
