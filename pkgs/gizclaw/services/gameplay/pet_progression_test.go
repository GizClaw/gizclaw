...
func TestEmptyPetDriveSettlesAndPersistsTime(t *testing.T) {
	...
	adopted, err := runtime.AdoptPet(ctx, "peer-empty-drive", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
...
func TestEmptyPetDriveTimeSettlementIsFrequencyIndependent(t *testing.T) {
	ctx, runtime, now := newPetRuntime(t)
	first, err := runtime.AdoptPet(ctx, "peer-empty-frequency", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet(first) error = %v", err)
	}
	second, err := runtime.AdoptPet(ctx, "peer-empty-frequency", apitypes.PetAdoptRequest{Name: "pet-second", DisplayName: "Pet"})
	if err != nil {
...
func TestEmptyPetDriveDeathSettlementIsFrequencyIndependent(t *testing.T) {
	ctx, runtime, now := newPetRuntime(t)
	start := *now
	first, err := runtime.AdoptPet(ctx, "peer-empty-death", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet(first) error = %v", err)
	}
	second, err := runtime.AdoptPet(ctx, "peer-empty-death", apitypes.PetAdoptRequest{Name: "pet-second", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet(second) error = %v", err)
	}
	for _, pet := range []apitypes.Pet{first.Pet, second.Pet} {
...
func TestEmptyPetDriveIdempotencyKeyPreventsRepeatedTick(t *testing.T) {
	ctx, runtime, now := newPetRuntime(t)
	first, err := runtime.AdoptPet(ctx, "peer-empty-key", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet(first) error = %v", err)
	}
	second, err := runtime.AdoptPet(ctx, "peer-empty-key", apitypes.PetAdoptRequest{Name: "pet-second", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet(second) error = %v", err)
	}
	key := "empty-tick"
...
func TestEmptyPetDriveFailureDoesNotConsumeIdempotencyKey(t *testing.T) {
	ctx, runtime, now := newPetRuntime(t)
	adopted, err := runtime.AdoptPet(ctx, "peer-empty-failure", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet() error = %v", err)
	}
...
func TestPetDriveRejectsBehaviorAndGameResultTogether(t *testing.T) {
	...
	adopted, err := runtime.AdoptPet(ctx, "peer-invalid-drive", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet() error = %v", err)
	}
...
func TestPetCareBehaviorUsesDeltaCapEnergyAndExperience(t *testing.T) {
	ctx, runtime, _ := newPetRuntime(t)
	adopted, err := runtime.AdoptPet(ctx, "peer-care", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet() error = %v", err)
	}
	key := "feed-once"
...
func TestUnconfiguredGameIsExactNoOp(t *testing.T) {
	ctx, runtime, now := newPetRuntime(t)
	adopted, err := runtime.AdoptPet(ctx, "peer-noop", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet() error = %v", err)
	}
...
func TestGameRewardFailureIsAtomicAndDeathIsTerminal(t *testing.T) {
	ctx, runtime, now := newPetRuntime(t)
	adopted, err := runtime.AdoptPet(ctx, "peer-atomic", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet() error = %v", err)
	}
	original := adopted.Pet
...
func TestGameIdempotencyKeyCannotCrossPets(t *testing.T) {
	ctx, runtime, _ := newPetRuntime(t)
	first, err := runtime.AdoptPet(ctx, "peer-game-idempotency", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet(first) error = %v", err)
	}
	second, err := runtime.AdoptPet(ctx, "peer-game-idempotency", apitypes.PetAdoptRequest{Name: "pet-second", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet(second) error = %v", err)
	}
	ctx = WithRewardEvaluator(ctx, rewardEvaluatorFunc(func(context.Context, RewardEvaluationRequest) (apitypes.GameRewardSpec, error) {
...
func TestGameRewardDropsZeroBadgeDeltaBeforeIdempotentReplay(t *testing.T) {
	ctx, runtime, _ := newPetRuntime(t)
	adopted, err := runtime.AdoptPet(ctx, "peer-zero-badge", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet() error = %v", err)
	}
	ctx = WithRewardEvaluator(ctx, rewardEvaluatorFunc(func(context.Context, RewardEvaluationRequest) (apitypes.GameRewardSpec, error) {
...
func TestRewardEvaluationIncludesConfiguredGameDefinition(t *testing.T) {
	ctx, runtime, _ := newPetRuntime(t)
	adopted, err := runtime.AdoptPet(ctx, "peer-reward-context", apitypes.PetAdoptRequest{Name: "pet-main", DisplayName: "Pet"})
	if err != nil {
		t.Fatalf("AdoptPet() error = %v", err)
	}
