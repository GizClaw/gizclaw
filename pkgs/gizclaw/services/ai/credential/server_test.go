...
func TestValidateCredentialBodyProviderShapes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "deepseek api key", provider: "deepseek", body: `{"api_key":"sk-deepseek"}`},
		{name: "deepseek missing api key", provider: "deepseek", body: `{}`, wantErr: true},
		{name: "deepseek blank api key", provider: "deepseek", body: `{"api_key":" "}`, wantErr: true},
