...
type rpcGameplayPixaDownloadService interface {
	PreparePetPixaDownload(context.Context, rpcapi.PetPixaDownloadRequest) (rpcapi.PetPixaDownloadResponse, io.ReadCloser, *rpcapi.RPCStatus, error)
	PrepareBadgeDefPixaDownload(context.Context, rpcapi.BadgeDefPixaDownloadRequest) (rpcapi.BadgeDefPixaDownloadResponse, io.ReadCloser, *rpcapi.RPCStatus, error)
}
...
func (s *rpcServer) handlePetPixaDownload(ctx context.Context, stream *rpcStream, req *rpcapi.RPCRequest) error {
	...
	if !ok || service == nil {
		return writeRPCErrorResponse(stream, req.Id, rpcapi.StatusCodeInternal, "gameplay service not configured")
	}
