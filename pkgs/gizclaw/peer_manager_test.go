...
func TestManagerActivatePeerRollsBackReservationOnEnsureFailure(t *testing.T) {
	...
	if got, ok := manager.PeerRegistration(key); !ok || got.RuntimeProfile.Id != "profile" {
...
func TestManagerPeerEventBrokerFollowsConnectionGeneration(t *testing.T) {
	...
	manager.SetPeerUp(key, oldConn)
	if err := manager.SetPeerEventBroker(key, oldConn, oldBroker); err != nil {
	...
	if err := manager.SetPeerEventBroker(key, oldConn, oldBroker); !errors.Is(err, ErrPeerConnNotActive) {
	...
	if err := manager.SetPeerEventBroker(key, newConn, newBroker); err != nil {
...
func TestManagerWorkspaceHistoryEventsReachWorkspaceOwnerOnly(t *testing.T) {
	ctx := t.Context()
	owner := giznet.PublicKey{11}
	...
	manager.Workspaces = staticWorkspaceService{workspace: apitypes.Workspace{
		Id:             "id-workspace-a",
		Name:           "workspace-a",
		OwnerPublicKey: &ownerText,
	}}
	outputs := map[giznet.PublicKey]*peerStreamLockedBuffer{}
	for _, key := range []giznet.PublicKey{owner, unrelated} {
		conn := &testGiznetConn{publicKey: key}
		...
		manager.SetPeerUp(key, conn)
		if err := manager.SetPeerEventBroker(key, conn, broker); err != nil {
			t.Fatalf("SetPeerEventBroker(%s): %v", key, err)
		}
		outputs[key] = output
	}

	manager.broadcastWorkspaceHistoryUpdated(ctx, "id-workspace-a", time.UnixMilli(1234))
	waitForPeerStreamBytes(t, outputs[owner])
	event := readLockedPeerStreamEvent(t, outputs[owner])
	updated := event.GetWorkspaceHistoryUpdated()
	if updated.GetWorkspaceKind() != eventpb.WorkspaceKind_WORKSPACE_KIND_WORKFLOW ||
		updated.GetWorkspaceName() != "workspace-a" || updated.GetLastUpdatedAtUnixMs() != 1234 {
		t.Fatalf("history event = %+v", event)
	}
	if outputs[unrelated].Len() != 0 {
		t.Fatal("unrelated peer received Workspace history invalidation")
	}
}
...
func TestManagerPeerRegistrationFollowsActiveConnection(t *testing.T) {
	...
	if err := profiles.BindOwnerProfile(t.Context(), key.String(), oldRegistration.RuntimeProfile.Id); err != nil {
		t.Fatalf("BindOwnerProfile(old) error = %v", err)
	}
	if profile, err := manager.runtimeProfileForOwner(t.Context(), key.String()); err != nil || profile.Id != "profile-old" {
		t.Fatalf("runtimeProfileForOwner() = %#v, %v", profile, err)
	}
	resources := (&PeerService{manager: manager}).peerResources(key)
	if profile := resources.RuntimeProfile(); profile == nil || profile.Id != "profile-old" {
	...
	if err := profiles.BindOwnerProfile(t.Context(), key.String(), newRegistration.RuntimeProfile.Id); err != nil {
		t.Fatalf("BindOwnerProfile(new) error = %v", err)
	}
	manager.SetPeerDown(key, oldConn)
	if registration, ok := manager.PeerRegistration(key); !ok || registration.RuntimeProfile.Id != "profile-new" {
	...
	if profile, err := manager.runtimeProfileForOwner(t.Context(), key.String()); err != nil || profile.Id != "profile-new" {
...
func TestPeerConnRetireSerializesConcurrentRegistration(t *testing.T) {
	...
	registration := runtimeprofile.Registration{RuntimeProfile: apitypes.RuntimeProfile{Id: "profile-race"}}
