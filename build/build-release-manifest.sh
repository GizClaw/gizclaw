#!/usr/bin/env bash
# Create deterministic formal-release metadata from a closed payload directory.
...
[[ -d "$asset_dir" && ! -L "$asset_dir" && -n "$tag" && -n "$source_commit" ]] || {
  echo "usage: $0 --asset-dir DIR --tag vMAJOR.MINOR.PATCH --source-commit SHA" >&2
  exit 2
}
...
while IFS= read -r name; do
  ...
  if [[ "$name" == *.deb ]]; then
    ...
    [[ "$package" == gizclaw && "$package_version" == "$version" && "$package_source" == "$source_commit" ]] || {
