...
`rpcproto` is generated directly from `api/proto/rpc/rpc.proto` and the split
`api/proto/rpc/payload/*.proto` files. The
...
do not add JSON-schema DTOs for those payloads under `api/http/shared/server.json`.
