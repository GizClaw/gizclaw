...
func (l *loader) loadModel(cfg Config) (Model, error) {
	...
	methodIDs, err := loadProtoMethodIDs(cfg.ProtoPath)
	if err != nil {
		return Model{}, err
	}
...
func loadProtoMethodIDs(path string) (map[string]int, error) {
	if path == "" {
		return nil, nil
	}
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		if prev := methodIDs[pending]; prev != 0 {
			return nil, fmt.Errorf("%s: duplicate rpc method %q", path, pending)
		}
		methodIDs[pending] = id
		pending = ""
	}
