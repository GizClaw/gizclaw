...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/locomo-credentials.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT
env_file="$tmp_dir/.env"
candidate="$tmp_dir/candidate.env"
marker="$tmp_dir/must-not-exist"

if require_locomo_e2e_credentials "$env_file" >/dev/null 2>&1; then exit 1; fi
# shellcheck disable=SC2154
for name in "${locomo_e2e_credentials[@]}"; do printf '%s=test-value\n' "$name" >>"$env_file"; done
...
for name in "${locomo_e2e_credentials[@]}"; do
  ...
  if require_locomo_e2e_credentials "$candidate" >/dev/null 2>&1; then exit 1; fi
done
