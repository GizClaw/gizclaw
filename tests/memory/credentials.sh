...
memory_e2e_credentials=(GIZCLAW_MEMORY_E2E_API_KEY)

require_memory_e2e_credentials() {
  local env_file="$1"
  ...
  for name in "${memory_e2e_credentials[@]}"; do
    unset "$name"
  done
  for name in "${memory_e2e_credentials[@]}"; do
    value=""
  ...
  if ((${#invalid[@]} > 0)); then
    printf 'invalid or missing Memory E2E credentials in %s:\n' "$env_file" >&2
    printf '  %s\n' "${invalid[@]}" >&2
    return 2
  fi
}
