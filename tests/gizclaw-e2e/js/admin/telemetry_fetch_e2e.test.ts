...
async function main(): Promise<void> {
  ...
  const { packetChannel, pc: peerPC } =
    await connectSetupPeerWithPacketChannel(peerIdentityDir);
  ...
  try {
    ...
    assert.ok(
      ranged.data.points.length >= 6,
      `range point count ${ranged.data.points.length}`,
    );
    ...
    assert.ok(
      aggregate.data.points.length > 0,
      "aggregate points should not be empty",
    );
  } finally {
  ...
  return {
    ...
    observations: [
      batteryTelemetry({
        charging: index % 2 === 0,
        percent: 60 + index,
        voltageMv: 3700 + index * 7,
      }),
      gnssTelemetry({
        accuracyM: 3.5 + index / 10,
        altitudeM: 12 + index,
        latitude: 37.77 + index / 1000,
        longitude: -122.42 + index / 1000,
      }),
      networkTelemetry({
        connected: true,
        rssiDbm: -72 + index,
        signalLevel: 2 + (index % 4),
      }),
      systemTelemetry({
        freeMemoryBytes: 64 * 1024 * 1024 - index * 128 * 1024,
        temperatureC: 35.5 + index / 10,
        uptimeSeconds: 3600 + index * 120,
      }),
    ],
    sequence: index + 1,
  };
...
async function pollLatest(
  ...
  for (;;) {
    const latest = await getPeerTelemetryLatest({
      ...
      query: {
        fields:
          "battery.percent,gnss.latitude,gnss.longitude,network.rssi_dbm,system.temperature_c",
      },
      throwOnError: true,
    });
    const battery = latest.data.values.find(
      (value) => value.field === "battery.percent",
    );
    const latitude = latest.data.values.find(
      (value) => value.field === "gnss.latitude",
    );
    const rssi = latest.data.values.find(
      (value) => value.field === "network.rssi_dbm",
    );
    const temperature = latest.data.values.find(
      (value) => value.field === "system.temperature_c",
    );
    if (
      battery?.value === 71 &&
      latitude != null &&
      Math.abs(latitude.value - 37.781) < 0.000001 &&
      rssi?.value === -61 &&
    ...
    if (Date.now() > deadline) {
      throw new Error(
        `latest telemetry did not become ready: ${JSON.stringify(latest.data.values)}`,
      );
    }
...
main().then(
  () => {
    console.log(
      "ok - Node admin SDK queries peer telemetry written through telemetry packets",
    );
    process.exit(0);
  },
  (err: unknown) => {
