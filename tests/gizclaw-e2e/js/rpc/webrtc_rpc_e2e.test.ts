...
import { assertSetupServerAvailable, closePeerConnection, connectSetupPeerWithPacketChannel, loadIdentity, repoRoot } from "../common/webrtc.ts";
...
async function main(): Promise<void> {
  ...
  const { packetChannel, pc } = await connectSetupPeerWithPacketChannel(identityDir);
  try {
...
async function pollServerStatus(rpc: ReturnType<typeof createPeerRPCClient>): Promise<{ battery_percent?: number; charging?: boolean }> {
  const deadline = Date.now() + 5000;
  for (;;) {
    const status = (await rpc.call("server.status.get", {})) as { battery_percent?: number; charging?: boolean };
    if (status.battery_percent === 88 && status.charging === true) {
      return status;
    }
    if (Date.now() > deadline) {
      throw new Error(`server.status.get did not reflect telemetry: ${JSON.stringify(status)}`);
    }
...
main().then(
  () => {
    console.log("ok - Node WebRTC SDK connects to setup server, runs all.ping, and reports telemetry");
    process.exit(0);
  },
  (err: unknown) => {
    console.error(err);
