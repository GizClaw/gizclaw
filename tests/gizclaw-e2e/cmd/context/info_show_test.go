...
func TestContextInfoAndShowUserStory(t *testing.T) {
	...
	info.MustSucceed(t)
	for _, want := range []string{`"name":"beta"`, `"current":true`, `"server_endpoint":"http://127.0.0.1:9821"`} {
	...
	show.MustSucceed(t)
	for _, want := range []string{`"name":"alpha"`, `"current":false`, `"server_endpoint":"http://` + h.ClientEndpoint() + `"`} {
