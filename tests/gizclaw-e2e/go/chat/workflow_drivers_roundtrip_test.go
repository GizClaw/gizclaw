...
func TestEinoWorkflowRoundtrip(t *testing.T) {
	runConfiguredMemoryRoundtrip(t, "eino-memory.json", workspaceCaseTextRoundtrip)
}

func TestFlowcraftConfiguredMemoryStoreRoundtrip(t *testing.T) {
	runConfiguredMemoryRoundtrip(t, "flowcraft-configured-memory.json", workspaceCaseTextRoundtrip)
}

func runConfiguredMemoryRoundtrip(t *testing.T, configName string, selectedCase workspaceCase) {
	t.Helper()
	...
	if _, err := runLoadedConfigWithResultAndInspect(cfg, selectedCase, func(ctx context.Context, client *gizcli.Client, applied config) error {
		return waitConfiguredMemoryRecall(ctx, client, applied, uniqueToken)
	}); err != nil {
...
func waitConfiguredMemoryRecall(ctx context.Context, client *gizcli.Client, cfg config, query string) error {
	...
	for {
		...
		if stats.Backend == nil || *stats.Backend != "mem0" {
			return fmt.Errorf("memory backend = %v, want mem0", stats.Backend)
		}
