...
func (latencyFixtureBuilder) BuildGenerator(context.Context, GeneratorConfig) (genx.Generator, error) {
	return nil, errors.New("slow-tts fixture uses deterministic workflow script nodes")
}
...
func (t latencyTTS) Transform(ctx context.Context, input genx.Stream) (genx.Stream, error) {
	fmt.Fprintln(os.Stderr, "slow-tts: startup", t.startup)
	...
	go func() {
		...
		// Announce the session before synthesis. Combining BOS with the first
		// packet also races the independent Peer event and RTP channels; that
		// transport ordering is outside this TTS text-latency regression.
		id := genx.NewStreamID()
		if err := out.Add(&genx.MessageChunk{Role: genx.RoleModel, Part: &genx.Blob{MIMEType: "audio/opus"}, Ctrl: &genx.StreamCtrl{StreamID: id, BeginOfStream: true}}); err != nil {
			return
		}
		...
		for i := 0; i < 80; i++ {
			...
			if err := out.Add(&genx.MessageChunk{Role: genx.RoleModel, Part: &genx.Blob{MIMEType: "audio/opus", Data: packet}, Ctrl: &genx.StreamCtrl{StreamID: id}}); err != nil {
				return
			}
		}
