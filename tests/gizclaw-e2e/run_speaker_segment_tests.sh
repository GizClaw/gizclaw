#!/usr/bin/env bash
# Model-free speaker-segments Giztests against an isolated real Server and Edge.
set -euo pipefail
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_dir="$(cd "$script_dir/../.." && pwd)"
cd "$repo_dir"
mkdir -p "$script_dir/.testbench"
run_dir="$(mktemp -d "$script_dir/.testbench/speaker-segments-XXXXXX")"
...
paths = [root / "pkgs/genx/agentkit/audiodock/dock.go", root / "pkgs/genx/agentkit/audiodock/speakers.go", root / "cmd/internal/commands/giztest/peer_stream.go", root / "tests/gizclaw-e2e/testdata/slow-tts/provider.go", root / "tests/gizclaw-e2e/testdata/slow-tts/overlay.py"]
paths += sorted((root / "tests/gizclaw-e2e/giztest").glob("speaker-segments.*.giztest.yaml"))
paths += sorted((root / "tests/gizclaw-e2e/testdata/speaker-segments").glob("*"))
output.write_text(json.dumps({
    "revision": subprocess.check_output(["git", "-C", str(root), "rev-parse", "HEAD"], text=True).strip(),
    "tts_startup_ms": 100,
    "tts_synthesis_ms": 100,
    "sha256": {str(p.relative_to(root)): hashlib.sha256(p.read_bytes()).hexdigest() for p in paths if p.is_file()},
}, indent=2) + "\n")
PYMANIFEST
project="gizclaw-speaker-segments-test-$$"
...
services:
  server:
    user: "$run_user"
    environment: {SPEAKER_SEGMENTS_FIXTURE: "1"}
  edge: {user: "$run_user"}
  seed: {user: "$run_user"}
  test:
    user: "$run_user"
...
cleanup() {
  ...
  echo "Speaker segment E2E reports: $GIZCLAW_MONITOR_REPORTS"
}
trap cleanup EXIT
arch=amd64
case "$(docker info --format '{{.Architecture}}')" in arm64 | aarch64) arch=arm64 ;; esac
npm ci --offline && npm run --offline build:console
...
cp -R "$script_dir/testdata/speaker-segments" "$image_dir/tests/gizclaw-e2e/testdata/"
