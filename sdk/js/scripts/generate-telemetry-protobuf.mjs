...
const enums = new Map(
  [...proto.matchAll(/enum\s+(\w+)\s*\{([^}]+)\}/g)].map((match) => [
    match[1],
    [...match[2].matchAll(/(\w+)\s*=\s*(\d+)\s*;/g)].map((value) => [
      value[1],
      Number(value[2]),
    ]),
  ]),
...
${[...enums].map(([name, values]) => `export const ${name} = {\n${values.map(([key, value]) => `  ${key}: ${value},`).join("\n")}\n} as const;\nexport type ${name} = number;`).join("\n\n")}
...
function writerMethod(type, messages) {
  if (enums.has(type)) return "int32";
  switch (type) {
    case "uint32":
    case "int32":
    case "int64":
