...
void main() {
  ...
  final tools = _parseRpcMethods(
    File('$repo/api/proto/rpc/payload/tool.proto').readAsStringSync(),
    prefix: 'CLIENT_TOOL',
    extension: 'client_tool',
  );
...
void _writePayloadBarrels(String outDir, List<String> payloadFiles) {
  ...
  for (final suffix in ['pb.dart', 'pbenum.dart', 'pbjson.dart']) {
    ...
    for (final library in libraries) {
      ...
      if (suffix == 'pb.dart' && library == 'tool') {
        // The extension namespace is also named Tool; ai.pb.dart owns the
        // public Tool resource message in this barrel.
        buffer.writeln("export 'payload/tool.pb.dart' hide Tool;");
        continue;
      }
...
List<_Method> _parseRpcMethods(
  String proto, {
  String prefix = 'RPC_METHOD',
  String extension = 'rpc_method',
}) {
  ...
  final pattern = RegExp(
    r'^\s*RPC_METHOD_[A-Z0-9_]+\s*=\s*(\d+)\s*\[\(rpc_method\)\s*=\s*\{\s*name:\s*"([^"]+)"\s+request:\s*"(\w+)"\s+response:\s*"(\w+)"\s*\}\s*(?:,\s*deprecated\s*=\s*(true|false)\s*)?\]\s*;'
        .replaceAll('RPC_METHOD', prefix)
        .replaceAll('rpc_method', extension),
    multiLine: true,
  );
...
void _writeMethodRegistry(
  ...
  for (final tool in tools) {
    buffer.writeln(
      "'${tool.name}': RpcMethodDescriptor(id: ${tool.id}, name: '${tool.name}', requestType: '${tool.request}', responseType: '${tool.response}'),",
    );
  }
  buffer.writeln('};');
  buffer.writeln('const clientToolNamesById = <int, String>{');
  for (final tool in tools) {
    buffer.writeln("${tool.id}: '${tool.name}',");
  }
  buffer.writeln('};');
  buffer.writeln(
    "RpcMethodDescriptor clientToolByName(String name) => clientToolsByName[name] ?? (throw ArgumentError.value(name, 'name', 'unknown client tool'));",
  );
  buffer.writeln(
    "RpcMethodDescriptor clientToolById(int id) => clientToolByName(clientToolNamesById[id] ?? (throw ArgumentError.value(id, 'id', 'unknown client tool')));",
  );
...
void _writePayloadCodec(
  ...
  buffer.writeln(
    "GeneratedMessage decodeClientToolRequestPayload(int tool, List<int> bytes) => newPayloadMessage(clientToolById(tool).requestType)..mergeFromBuffer(bytes);",
  );
  buffer.writeln(
    "GeneratedMessage decodeClientToolResponsePayload(int tool, List<int> bytes) => newPayloadMessage(clientToolById(tool).responseType)..mergeFromBuffer(bytes);",
  );
  buffer.writeln(
    "List<int> encodeClientToolRequestPayload(int tool, GeneratedMessage message) { _expectPayloadType(clientToolById(tool).requestType, message); return message.writeToBuffer(); }",
  );
  buffer.writeln(
    "List<int> encodeClientToolResponsePayload(int tool, GeneratedMessage message) { _expectPayloadType(clientToolById(tool).responseType, message); return message.writeToBuffer(); }",
  );
  File(
