...
- Local Pod: The desktop version maintains a local Server, and the port remains stable after creation; the Server
  ...
  `RuntimeProfile/default`. Its Credential, Tenant, Model, Voice, and Workflow dependencies are resolved from the
  fixed Raids `v0.1` archive before that profile is applied. Desktop creates
