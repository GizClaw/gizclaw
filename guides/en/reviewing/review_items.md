...
- Whether the repository-pinned `modernize` was run through `tools/quality` and every handwritten Go diagnostic in every maintained module was fixed; whether `modernize.exemptions` contains only complete diagnostics currently emitted by the analyzer whose source is proven by a standard pre-package generated marker, an explicit generator-owned output rule, or an explicit third-party root; whether handwritten, unknown, missing, untracked, external, malformed, stale, wildcard, and path-only entries are rejected; whether repository-owned generator output is preferably fixed at the generator and third-party generated files remain unmodified. This file is an exemption list, not a baseline.
