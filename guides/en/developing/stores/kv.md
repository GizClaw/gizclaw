...
Peer records and routes share the Store named by `services.peer.store`. High-churn PeerRun state uses the separately required `services.peer_run.store`; that binding accepts any supported `keyvalue` backend and follows the backend's ordinary configuration and runtime semantics.

PostgreSQL mutations acquire transaction advisory locks scoped to the physical table and encoded keys, sorted by lock ID, including absent guards. Ordinary writes, deletes, and conditional mutations share this protocol without a table-wide write lock. Independent keys can progress concurrently; same-key and multi-key atomic operations remain coordinated. Hash collisions only add contention. PostgreSQL mutations clean up expired keys involved in that mutation; reads still treat expired values as absent. SQLite retains its existing transaction and expiration cleanup behavior.
