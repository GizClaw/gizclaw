...
The Prometheus driver coalesces concurrent `Append` calls per logical Store with a 5ms collection delay and ordinary batch limits of 4096 samples / 128 calls. Pending samples and estimated memory are bounded; admission waits for capacity or cancellation. A single oversized call occupies the budget alone. Success is reported only after remote-write acceptance, without automatic retries. One caller cancellation does not cancel other callers in the batch; cancellation of every caller or closing the Store cancels the backend request. Workers start on demand, exit when drained, and are joined by `Close`.
