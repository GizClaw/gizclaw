...
`services/ai/openaiapi` also maps one Conversation to one newly created caller-owned Workspace. Creation requires string metadata `workflow_name`, which is also the compatible Response model. Conversation items correlate to authoritative Workspace History, while lifecycle and immutable input snapshots are stored below the Workspace runtime prefix. Reads never start an Agent. A Response attaches the shared Workspace Agent only for that turn, supports foreground JSON/SSE or background cancellation, and leaves PeerRun selection unchanged.
