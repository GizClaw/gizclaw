...
`server.api_key.create` (method 96) is the only keyless bootstrap for a Public HTTP credential. It accepts only `display_name` and `manage_api_keys`; the owner is fixed to the active Client connection. The Server also verifies the durable RuntimeProfile owner binding and returns the complete secret only once in the successful response. Subsequent list/get/revoke operations use `/gizclaw/v1/api-keys*`.
