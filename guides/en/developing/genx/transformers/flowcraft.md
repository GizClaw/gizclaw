...
A new text BOS cancels an unfinished prior turn, removes its unpulled output, and emits an EOS with an `interrupted` error after that turn reaches its persistence boundary. History and Memory contain only assistant text that crossed output `Next()`; a discarded unpulled suffix is not recorded. An interrupted assistant History message carries an interruption data marker.
