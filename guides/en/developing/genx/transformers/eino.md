...
`Config.OutputMetadata` derives process-local `map[string]string` attributes from a detached turn state snapshot before each output route's first nonblank chunk. The callback must support concurrent turns. The route keeps its attributes fixed, and `MessageChunk.Metadata` carries them to in-process adapters; Clone copies the map and wire encoders must omit it. Leading whitespace does not trigger a snapshot. Without the callback, output does not copy state or wait for selection. This hook preserves Graph output names, primary ownership, History, and memory. Product adapters can use it to carry a selected Voice alias; the generic Eino package does not resolve Voice resources or perform TTS.
