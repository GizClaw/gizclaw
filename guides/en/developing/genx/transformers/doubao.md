...
Push-to-Talk classifies a turn from its accumulated, trimmed ASR hypothesis when the matching `ASREnded` arrives. An empty hypothesis is a successful no-input turn: the transformer publishes the empty transcript lifecycle, then complete empty assistant text and audio lifecycles for the same StreamID, and immediately returns the state machine to Idle. It does not arm a response deadline, report provider loss, interrupt, or replace the healthy session. A later turn reuses that session normally. A non-empty hypothesis continues through the existing Push-to-Talk Chat/TTS response path; Push-to-Talk does not use the Realtime response deadline.
