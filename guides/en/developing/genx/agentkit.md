...
`ResolveVoice` receives the response StreamID, output node/name, and chunk metadata, and returns the pattern passed to the TTS mux. An empty pattern keeps text output without synthesizing that response. RuntimeProfile alias resolution belongs to the product factory, not Audio Dock.
