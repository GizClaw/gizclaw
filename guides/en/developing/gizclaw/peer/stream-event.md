...
For an Edge-routed Peer, `peerAgentOutput` binds output to a logical turn only through the producer response epoch's immutable input-route provenance. The association survives input replacement, so even the first late observation of an old response remains on the old `turn_index`; a self-started or third-party response without provenance never falls back to the current turn. The response-complete boundary, not the first MIME EOS, records the bounded `output_terminal` and completes the owning turn.

Produced accounting classifies the GenX source chunks. Delivered accounting classifies only actual `PeerEvent` values after `Broadcast` succeeds; aggregate audio is counted only after mixer drain. Failed broadcast, failed or abandoned drain, and suppressed aggregate audio boundaries do not add a delivered modality. Empty-label text and blob events keep the Peer client's assistant fallback, while an empty-label control-only EOS remains `other`. Terminal snapshots contain only sorted closed classes for source part and label, Peer event type, kind, and label; payloads and raw labels remain excluded.

When workspace replacement cancels an old response, user BOS interrupts an agent opening, or a new assistant BOS with the same label cuts over the mixed-audio epoch, the old output ends with an error-free EOS. Clients recognize barge-in from the new BOS, not from the old EOS. Actual model and audio processing failures retain their error code and message. Input route reload still uses `INPUT_ROUTE_RELOADED` to request fresh input.
