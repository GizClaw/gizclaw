...
When workspace replacement cancels an old response or user BOS interrupts an agent opening, the old output ends with an error-free EOS. Actual model and audio processing failures retain their error code and message. Input route reload still uses `INPUT_ROUTE_RELOADED` to request fresh input.
