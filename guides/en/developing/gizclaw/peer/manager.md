...
After Admin block persists `blocked`, `DetachPeerConnections` removes the current connection, Edge transports, and activation reservation under same-key record coordination, then closes captured transports after releasing both record and Manager locks. Late activation cannot publish a detached reservation. No permanent fence remains: Admin approve restores active status and permits a fresh connection immediately; old teardown cannot close that new generation. `ForcePeerDown` retains its existing index-only cleanup and replacement-reservation behavior rather than implementing block disconnection. Activation rejection precedes PeerRoutes assignment and LocalRuns writes. Bounded checks for new service streams are described in [Security Policy](../server/security-policy).
