...
`peer_http_api_key.go` adapts `services/system/apikey` to the generated Peer HTTP contract. Bearer authentication is performed before the strict handlers run, and the authenticated key's owner becomes the Peer identity used by GizClaw and OpenAI-compatible services.
