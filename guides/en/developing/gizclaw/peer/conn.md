...
Before any RPC, HTTP, Event, packet, or audio loop starts, `PeerConn` atomically ensures its durable Peer generation and publishes the exact connection in `Manager`; therefore an immediate `server.register` cannot precede connection activation. After `server.peer.delete` commits the active Peer deletion, `PeerConn` immediately enters retiring state: it conditionally detaches that exact connection and its registration from `Manager`, rejects new work, and then attempts to write the deletion acknowledgement and EOS. The current delete RPC transport remains available until that write attempt finishes. The terminal action closes the full Giznet connection even when the response or EOS write fails. A replacement connection for the same public key is not removed by, and cannot be deleted through, the retiring or close path of the old connection.
