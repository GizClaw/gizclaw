...
The downlink encoder is a 16 kHz mono libopus encoder in `ApplicationAudio` with complexity fixed at 0. The Server encodes one stream per connected Peer, so the libopus default of 9 spends CPU per connection for quality the 16 kHz mono mixed downlink does not carry. The complexity is not configurable and does not adapt to load.
