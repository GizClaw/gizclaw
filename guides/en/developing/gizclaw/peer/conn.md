...
Friend and Friend Group SFU Workspaces have a dedicated inbound authorization path. When an input BOS arrives, `PeerConn` asks `Manager.sfuInputAccess` to resolve the selected Workspace name to its SFU binding in the authoritative Social KV (never through the Server-local Workspace catalog, whose name index is scoped by owner) and checks membership: input for non-SFU Workspaces is admitted as before, while input for an SFU Workspace is admitted only when the check passes, the Workspace has not been revoked on this connection, and the current runtime is attached; otherwise it is rejected with `SFU_ACCESS_REVOKED`, `SFU_ACCESS_CHECK_FAILED`, or `SFU_RUNTIME_NOT_ATTACHED`. Opus packets for a revoked Workspace are dropped and counted, never buffered. The connection receives no revocation push: an established participant is terminated by the SFU runtime's own periodic re-validation (see [services/social](/en/developing/gizclaw/services/social#revocation)), after which the inbound check rejects new turns because the membership is gone. SFU Workspaces are the only Workspaces admitted to non-owner restricted reload; that decision uses Social binding membership rather than owner rights.
