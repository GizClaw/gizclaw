...
| `MixerOutput` | Decode Agent audio into PCM on one mixer track per `(StreamID, canonical MIME)`; MIME EOS closes only that track, while control-only EOS closes every track on the route. Chunks marked `OpusPassthroughMIME` (`audio/opus; passthrough=1`) are neither decoded nor given a track: they go to the consumer-installed `Passthrough` hook but are still observed in order. |
...
One History entry keeps a route's audio in arrival order. When the audio MIME type changes within a route, such as a multi-voice reply mixing Ogg/Opus and MP3 providers, each MIME run becomes its own segment; persistence converts every segment to Opus packets (Ogg/Opus unpacked, MP3 decoded, PCM encoded) and stores them as one `audio/ogg; codecs=opus` asset. History recording is best-effort and never fails the live output: a chunk that cannot be recorded, an entry that cannot be encoded, or a failed History append drops only that entry and logs a `workspace history entry dropped` warning, while the stream continues to the device and other entries are still stored.
...
Transformers and history replay drain provider output into growable stream buffers without waiting on a playback clock. Raw Opus, Ogg/Opus, MP3, and PCM audio are decoded or normalized before entering the mixed PCM stream; `PeerConn` reads one frame at each 20 ms pacing opportunity, encodes Opus, and writes it to WebRTC. The one exception is passthrough Opus (`agenthost.OpusPassthroughMIME`): `MixerOutput.Passthrough` claims those chunks before any decoding and `PeerConn` writes each payload unchanged to the Device Opus track, outside the mixer and the pacer; the mixer writes nothing to the Device while it owns no track, so a passthrough route is never interleaved with mixer silence. Normal EOS uses `CloseWrite` so buffered PCM drains, while error EOS uses `CloseWithError` to discard the matching track and its unconsumed stream backlog. Each `(StreamID, MIME)` pair is its own concurrently mixed track and MP3 is decoded only at its EOS, so producers keep one audio MIME type per reply route; AudioDock enforces this for speaker segments.

A mixer track is realtime from the moment it exists: the mixer fills a track with no buffered audio with digital silence, the pacer delivers that silence into the device buffer faster than real time, and the device must play it before the reply can be heard. `MixerOutput` therefore opens no track until a route has decoded **audible** PCM. A header-only Ogg page, a still-buffered MP3 body, or any other blob that decodes to nothing opens no track, and the low-level lead-in provider TTS emits before speech (sample magnitude below 256, about -42 dBFS) is dropped at the route start, keeping a 60 ms preroll ahead of the first audible sample so a soft onset is not clipped. A route drops at most 2 s; quiet that lasts longer is treated as deliberate and plays from there on, and quiet after the first audible sample (the pauses between sentences) is left unchanged. A route that never produces an audible sample opens no track.
...
A replacement route BOS can arrive after reload ends the old route but before the replacement runtime is published. Input admission waits up to two seconds for the local transition, then authorizes the published revision. Cancellation or timeout still denies input. Waiting does not query Redis. Audio packets from the old route remain fenced by their original revision and cannot inherit the new route permission.
