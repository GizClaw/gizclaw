...
Friend creation, Friend retirement, and Friend Group retirement each maintain a dedicated Set recovery index. Work IDs are hashed across 256 sets, with a directory of at most 256 bucket names. Empty buckets may remain in the directory to avoid racing directory removal against new work. Records and index memberships are published or removed in the same atomic mutation. Recovery reads pending IDs one bucket at a time and re-reads their records; it does not scan business keys or retain completed IDs in the member sets.

The Admin Friend list retains cross-owner pagination. Current Friend row identities live in 256 partitioned ordered collections, with a directory containing at most 256 bucket names. Creation and deletion atomically update both owner-view index entries with the relationship records. Cursors are opaque and order rows by bucket, then owner/relation ID within the bucket. Index ranges and result limits are evaluated by the storage backend. Listing reads only current-page records and reuses Workspace binding reads within each page.

A Peer has at most 10 Friends, counting relationships it initiated and relationships others initiated with it. The cap is fixed by `socialutil.PeerFriendLimit` and is not configurable through RuntimeProfile or Server config. Creation checks both Peers before creating the Workspace and again when committing the relationship: each creation atomically advances a per-Peer Friend revision, and the commit compares both Peers' revisions, so concurrent creations on different Servers cannot exceed the cap. When either Peer is full, the `friend.add` RPC returns `RESOURCE_EXHAUSTED` (8) with reason `FRIEND_LIMIT_REACHED`, and Admin HTTP creation returns `409 Conflict` with error code `FRIEND_LIMIT_REACHED`. A pending creation that no longer fits at commit time is cancelled and its Workspace deleted. Re-adding an existing Friend still succeeds.
...
The Social service commits the relationship change in one `BatchMutate` (incrementing `generation` when the binding itself is replaced) and pushes no cancellation signal to any Peer or Server. The SFU runtime terminates itself: it re-reads its binding in the shared Social KV every `services.sfu.recheck_interval` and fails closed as soon as the membership is gone, the generation differs, or the resolver errors, disconnecting the participant and ending the session. Local and remote Peers behave identically, so revocation is eventually consistent and bounds stale membership by the refresh interval plus a request timeout of `min(recheck_interval, 2s)`, seven seconds by default. Refresh errors and timeouts stop forwarding immediately. BOS and EOS use a connection-local permission snapshot refreshed in the background per runtime revision. Opus packets check the local accepted stream, permission expiry, and AgentHost revision without database reads. The Workspace deletion cleaner's asynchronous quiesce still performs the final release.
...
Friend and Friend Group resolve bindings through exact Workspace ID and generated canonical-name locators. Locators are written atomically with the binding and retain the original Workspace identity after retirement. A missing or replaced binding revokes the old Workspace instead of resolving it to a new room. Resolution does not scan bindings, retirement intents, or receipts.
