...
Gameplay uses Workspace ownership and the Pet domain relationship. It does not create extra roles or policy bindings. Pet deletion atomically removes only the active Pet row and writes one `kind=pet` PendingDeletion in the same gameplay SQL database. The Pet's bound system Workspace stays active and visible, and no Workspace pending record is created. Points, badges, results, transactions, and reward-grant history are preserved.
