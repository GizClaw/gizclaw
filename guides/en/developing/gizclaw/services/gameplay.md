...
The Server initializes and validates the Gameplay SQL schema at startup. Hosts using `Runtime` directly must call `Migration` before serving requests or starting dispatchers. Business queries and background tasks do not create or repair the schema.

The catalog uses separate SQL business tables: `pet_definitions`, `badge_definitions`, and `game_definitions`. `services.gameplay.store` selects its SQL connection. IDs, prompts, display names, descriptions, reward prompts, and asset paths have separate columns; visual configuration, metadata, tags, and icon configuration remain JSON. Tables are created at startup. Lists use ID cursors and SQL limits without KV prefix enumeration. Updates and deletes compare revision and creation identity to reject concurrent changes or recreated records. Catalog and runtime data can share a connection pool; Badge definition reads inside reward transactions reuse the current transaction.

RuntimeProfile catalog existence checks deduplicate IDs and query batches of at most 256 IDs, reading only the ID column. Duplicate bindings add no database reads, and empty collections issue no query.

Background deletion discovery uses separate `(kind, task_status, next_attempt_at, deletion_id)` and `(kind, task_status, lease_deadline, deletion_id)` indexes for due retries and expired leases. Discovery reads only task IDs and fingerprints, paginated by ID; claiming reads the full record. Deletion status checks query the locator directly without scanning old records to recover indexes.
...
Pet adoption creates its Workspace before opening the points and Pet transaction, then rechecks the balance inside that transaction. Failure removes only the Workspace created by that attempt using its owner scope, avoiding nested connection acquisition with a shared single-connection database.
An idempotent retry reads a consistent Pet, transaction, and balance snapshot, then releases that read transaction before checking the Workspace binding.
...
Pet adoption first validates the current connection's RuntimeProfile and the canonical Workflow ID in `workflows.system.pet`, then creates an owner-bound system Workspace and records the exact RuntimeProfile ID on the Pet and related state. A Pet Workspace stores no persona, conversation, model, voice, or other driver configuration. Generic Workspace put may change only its input mode between `push-to-talk` and `realtime`; all other execution configuration remains immutable. PetDef contains no Voice ID/alias or local i18n; it retains character/speaking style, PIXA, and behavior-to-animation bindings. Presentation text comes from the RuntimeProfile `pet_defs` binding.

`driver: pet` is a domain wrapper with no built-in execution graph. Its `pet` field contains a same-shaped reusable Workflow spec that may select `flowcraft`, `eino`, `doubao-realtime`, or `ast-translate`, but cannot recursively select `pet` and cannot select `sfu`. The wrapper injects current Pet context and converts the Pet Workspace input mode into the nested driver's typed parameters; omission defaults to `push-to-talk`. The nested driver owns graph, voice, model, and toolkit configuration. Memory is selected once by the outer Workflow and passed to the nested driver; the reusable inner spec cannot declare another Memory alias. Every alias resolves through the system Workspace owner's RuntimeProfile, so replacing the nested driver does not require changing Pet or Workspace data.
...
The registered `source=gameplay` processor handler later verifies the immutable Pet descriptor, marker fingerprint, exact deterministic marker classification, and live SQL lease. Its final transaction deletes only the matching retained Pet row, matching locator, and marker/task state. An unrecognized marker identity is rejected before finalization; there is no legacy-marker completion path. The Pet-to-Workspace binding and bound system Workspace remain, as do Points, badges, results, transactions, rewards, Drive Fact outbox, Workspace reward state, Social data, prompts, messages, and history. Completion leaves no task receipt, and zero-row Pet mutations cannot report success after cleanup wins a race.
...
Exact-Workspace activation, History paging, model evaluation, and retry dispatch all use the same authoritative availability gate. Once the Workspace or owner is pending deletion, or the canonical Workspace has already been physically removed, stale activations complete idempotently: unsettled windows and the exact source row are removed, completed reward audit rows remain, and no further History or model call is made or operational error is emitted for that Workspace. PendingDeletion marker creation and final settlement also serialize through the existing per-Workspace reward source row: a settlement committed before marker creation remains valid history, while a marker that wins the fence prevents any later RewardGrant, Points, or badge commit. The database fence is durable across processes and restarts; unrelated Workspaces retain independent ordering. When Workspace and Gameplay share a SQL pool, deletion marker writes and settlement availability checks reuse the current reward transaction, including pools limited to one connection. Existing pending, retry, and expired claims resume directly from the indexed Gameplay SQL queue after restart; no Workspace catalog scan is involved.
