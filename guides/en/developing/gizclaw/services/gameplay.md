...
Gameplay owns PetDef, BadgeDef, GameDef, Pet, points accounts, transactions, reward grants, badge progression, and game results. RuntimeProfile `resources.pet_defs`, `resources.game_defs`, and `resources.badge_defs` maps provide profile-local aliases used by `gameplay.adoption.pool` and `gameplay.rewards`.
