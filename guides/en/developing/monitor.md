...
The frontend uses `@gizclaw/gizclaw-control` for Peer Monitor, Node Monitor and public discovery. The SDK reuses generated Peer HTTP contracts and owns the generated Node Monitor client. Run `bash tests/gizclaw-e2e/run_monitor_tests.sh` for real API acceptance; see [Monitor API giztest](testing#monitor-api-giztest).
