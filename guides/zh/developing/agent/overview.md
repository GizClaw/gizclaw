...
`pkgs/agent` 不依赖 GizClaw Resource、ACL、Workspace、Peer、RPC 或生成 API。产品层先解析 model、store、授权和 device-bound executor，再把完成解析的依赖传给具体 Agent Config。每个实现拥有自己的强类型 Config；不存在跨 provider 的 union Config。

## Toolkit 与 ToolCall

Agent-facing Toolkit 同时提供不可变的 Tool declaration snapshot 和对应的 `Invoke`。GizClaw 只把通过 enabled state、Workflow/Workspace policy、ACL `use`、executor availability 和 device binding 检查的 Tool 放入 Toolkit；调用名称还必须存在于该 snapshot。执行时会重新检查 ACL 和实时可用性，但不会执行构造后才出现、从未声明给模型的 Tool。
