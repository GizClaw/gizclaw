...
API Key 用户可以通过 `createGizClawControlClient(...).device` 使用同样的读取方法。`listWorkspaces` 接受可选的 `workflow_name` 过滤，`deleteWorkspace` 在 Server 返回 `202` 后 resolve（公钥 bearer 需要 fullcontrol）。
