...
`ResolveVoice` 接收 response StreamID、输出 node/name 与 chunk metadata，返回交给 TTS mux 的 pattern。返回空 pattern 会只保留文本，不合成该输出的音频。RuntimeProfile alias 的解析属于产品 factory，不属于 Audio Dock。
