...
`pkgs/genx/transformers` 将一个 `genx.Stream` 转换为另一个 Stream。Provider Adapters 负责外部 speech/realtime 协议；Stream Processing 负责 provider-neutral 的 TTS normalization、segmentation 和组合。
...
ASR、TTS、Realtime 和其他能力都实现同一个 `genx.Transformer`，并通过同一个 `Mux` 注册。Guide 不为能力类别定义额外的 facade、session factory 或 registry API。
