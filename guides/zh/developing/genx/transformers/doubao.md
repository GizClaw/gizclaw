...
Push-to-Talk 在匹配的 `ASREnded` 到达时，根据当前 turn 累积并 trim 后的 ASR hypothesis 分类。空 hypothesis 是成功的无输入 turn：transformer 先发布空 transcript lifecycle，再为同一 StreamID 发布完整的空 assistant text 与 audio lifecycle，并立即让状态机回到 Idle。它不会启动 response deadline、报告 provider loss、发送 interrupt 或替换健康 session；后续 turn 正常复用该 session。非空 hypothesis 继续走既有 Push-to-Talk Chat/TTS response 路径；Push-to-Talk 不使用 Realtime response deadline。
