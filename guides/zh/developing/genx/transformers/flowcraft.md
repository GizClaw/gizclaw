...
新的文本 BOS 会取消尚未完成的上一轮，删除该轮未 pull 的输出，并在该轮完成持久化边界后发送带 `interrupted` error 的 EOS。由于纯控制 BOS 不声明 MIME，Flowcraft 会立即把它视为下一轮文本输入；不应打断文本回复的非文本 route 必须使用携带 MIME Part 的 BOS。History 和 Memory 只记录已经跨过最终 delivery observation 边界的 assistant 文本；被删除或尚未真正交付的后缀不会被记录。被打断的 History assistant message 带 interruption data marker。
