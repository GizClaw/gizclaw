...
新的文本 BOS 会取消尚未完成的上一轮，丢弃该轮尚未完成的输入和未 pull 的输出，并在该轮完成持久化边界后发送带 `interrupted` error 的 EOS。由于纯控制 BOS 不声明 MIME，Flowcraft 会立即把它视为下一轮文本输入；不应打断文本回复的非文本 route 必须使用携带 MIME Part 的 BOS。History 和 Memory 只记录已经跨过最终 delivery observation 边界的 assistant 文本；被删除或尚未真正交付的后缀不会被记录。至少交付过一个 assistant 文本 delta 时，才保存被打断的 user/assistant History 对，并给 assistant message 添加 interruption data marker；不会保存只有 marker 的空 assistant message。
