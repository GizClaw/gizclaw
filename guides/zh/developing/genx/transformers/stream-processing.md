...
| `audio/codecconv.TTSAudioNormalizer` | 去除 streaming TTS audio 中重复的 provider container header。 |
...
Provider package 负责 SDK request 和 audio synthesis。Container normalization 位于 `pkgs/audio/codecconv`，使 Go `internal` 边界以外的调用方也能复用。
