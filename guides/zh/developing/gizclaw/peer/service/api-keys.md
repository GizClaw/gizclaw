...
`peer_http_api_key.go` 把 `services/system/apikey` 适配到生成的 Peer HTTP 契约。Bearer 鉴权在 strict handler 前完成，认证 Key 的 owner 作为 GizClaw API 和 OpenAI 兼容服务使用的 Peer 身份。
