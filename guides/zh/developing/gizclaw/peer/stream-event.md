...
| 文件 | 包含的功能 |
| --- | --- |
| `peer_stream_event.go` | 维护 Peer event subscriber/broadcast broker；在 `PeerStreamEvent` 与 GenX message chunk 之间双向转换；处理 text、control、blob/audio 事件；将 Agent output 编码为 event stream 或 raw Opus direct packet；控制 Opus 发送节奏并把收到的事件推回 Agent input source。 |
...
| `peerAgentOutput` | 消费 Agent output，将其转换为 events 或 raw Opus packets。 |
