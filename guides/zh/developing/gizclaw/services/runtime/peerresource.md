...
Firmware 不属于 RuntimeProfile name catalog。RegistrationToken 可以给 Peer 绑定一个 canonical Firmware ID；`server.register` 返回 scoped Firmware name，`server.firmware.get` 和 download 从 caller Peer 解析绑定但不暴露 ID。设备只在 download request 中选择 channel。Peer RPC 不提供 Firmware list。
