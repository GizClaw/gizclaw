...
| `Service.SetRunAgent` | 在与 reload、stop 相同的 transition 边界内持久化 pending Peer selection。 |
| `Service.RuntimeRevision` / `PushInputIfCurrentRevision` / `ReloadIfCurrentRevision` | 仅当 connection-scoped input 仍属于当前稳定 runtime revision 时允许写入或恢复。 |
...
每个 `Service` 为其单个 Peer 串行化 selection 写入、reload、stop 与每次 Realtime input push。transition 在生命周期工作前后改变 runtime revision；Realtime chunk 如果观察到 revision 已变化或正处于 transition 中，即为过期输入，必须丢弃，不能重新打开或进入新的 workspace。input recovery 只能对未变化的稳定 revision 执行 reload；pending selection 仅在它改变当前 workspace 时才抑制恢复，因此同 workspace 的 selection 仍可恢复 inactive source。该边界不串行化无关 Peer，也不替代共享 `RuntimeRegistry` 对 workspace agent 的 ownership。
