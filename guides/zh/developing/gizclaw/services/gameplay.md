...
Gameplay 使用 Workspace owner 和 Pet 领域关系，不创建额外 role 或 policy binding。Pet 删除在同一个 gameplay SQL database transaction 中只删除 active Pet row，并写入一条 `kind=pet` PendingDeletion。Pet 绑定的 system Workspace 保持 active 和可见，不创建 Workspace pending record；points、badge、result、transaction 和 reward grant 历史全部保留。
