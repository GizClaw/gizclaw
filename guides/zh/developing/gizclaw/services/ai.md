...
可选 named store `agent-memory` 实现 provider-neutral `memory.Store`，由 Server 注入 Flowcraft、Eino 和 pet runtime。每个注入视图按 peer owner、Workspace、resolved Workflow 和 Agent driver 分区；共享 Server store 的 runtime 无法观察或 recall 其他 runtime 的事实。History 保存按 turn 排序的交付记录；Memory 保存可 recall 的长期事实，两者不能互相替代。Flowcraft product factory 直接构造 repository-owned `flowcraft.Config`，不写 Cloud/Claw config，也不使用外部 Claw Memory lifecycle。
