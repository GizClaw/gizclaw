...
配置后，`default_voice` 和 `node_voices` 通过 RuntimeProfile Voice alias 为声明为 `text/plain` 的 Graph output 合成语音。`node_voices` 以 Graph output 引用的 node ID 为 key，并优先于 `default_voice`。Eino Workspace `input` 接受 `push-to-talk` 或 `realtime`，默认值为 `push-to-talk`；realtime ASR 会输出 interim transcript。两种 live-audio 模式都以 `realtime_pacing=false` 解析 ASR Model：设备 frame 已按真实时间到达，如果从 provider session 建连完成后的首次发送重新回放，会再次叠加连接耗时。factory 在构造 Agent 前校验全部 alias，并用 AudioDock 组合 Eino Transformer，不把音频行为下沉到 provider-neutral Eino package。
