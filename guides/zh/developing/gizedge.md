...
与 cleanup gate 均沿用较小档位的固定 contract。Load driver 固定并记录 `GOGC=100`，
避免扩大 1,000-way client heap 的 GC 周期后把 heap scan、CPU 和 tail latency 退化注入
长时间 hold；这个参数不改变 Edge、Server 或 Coturn 的 runtime behavior。
