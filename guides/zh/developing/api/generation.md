...
Monitor OpenAPI（`api/http/monitor.json`）通过 `go generate ./pkgs/monitor/api` 生成 `pkgs/monitor/api/generated.go` 中的 strict Go server/client/models，配置位于 `pkgs/monitor/api/codegen_config.yaml`。`npm --prefix sdk/js run gen:sdk` 依据 `sdk/js/openapi-ts.config.ts` 生成 `sdk/js/gizclaw-control/generated/monitor/` 中的控制台 client。这些已提交输出归 Monitor surface 所有，由 `pkgs/monitor/monitor.go` 和 `web/monitor/src/api.ts` 使用，通过 `@gizclaw/gizclaw-control` 导出 Node Monitor client；Peer Monitor 方法在 control SDK 中复用生成的 Peer HTTP 契约。
