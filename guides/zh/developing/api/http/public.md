...
- `GET /device/runtime-profile` 返回 owner 当前绑定的 RuntimeProfile 的 `name`、`revision`，以及按 name 排序的 `workflows[]`（name、tags）。重复传 `tags` 查询参数时只返回包含全部请求字符串的 Workflow。`resource_id`、i18n、driver、models、voices、memory、pet 定义与 `app_config` 都不返回；binding 指向的 Workflow 资源是否仍存在不在读取时校验。绑定在鉴权后消失同样返回 `403 API_KEY_OWNER_UNAVAILABLE`。
- `GET /device/workspaces` 返回 owner 明确拥有的 Workspace（含系统 Workspace），不含共享、无 owner 与已进入 pending deletion 的 Workspace。每项返回 `id`、`name`、`workflow_name`、`available`、`system` 和时间字段；Admin Workflow ID 不返回。alias 不再解析时省略 `workflow_name` 且 `available=false`。可选 query `workflow_name` 精确过滤；空过滤值返回 `400 INVALID_REQUEST`。
