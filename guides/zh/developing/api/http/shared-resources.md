...
Workflow 与 Workspace 都不拥有本地 catalog 型 i18n。可选择 Workflow 的展示文本来自 RuntimeProfile binding 的 `i18n`，因此同一个真实 Resource 可以在不同 RuntimeProfile 中拥有不同文案。Workspace 没有 RuntimeProfile catalog binding；客户端根据拥有它的产品 surface 与稳定 Workspace 数据决定展示方式。
...
“视觉内容”不自动等于 `display`。如果 asset、clip、animation graph 或 behavior/state-to-clip mapping 被设备、runtime 或领域逻辑直接消费，它就是 Resource 的核心内容或关联数据。例如 Workspace icon 的 PIXA 数据属于 Workspace 内容，而可选择 Workflow 面向用户的本地化文本属于 RuntimeProfile binding。
