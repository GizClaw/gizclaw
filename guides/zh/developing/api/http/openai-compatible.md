...
准确的 endpoint、参数、request 和 response 见 [API Reference](/api/)。兼容目标仅限 Reference 中列出的 endpoint 与 payload，不表示实现全部 OpenAI API。新增字段或 endpoint 必须由 GizClaw 实际能力支持，不能只扩展 schema 而留下 placeholder handler。
