...
设备控制与 Contact 的 Public HTTP 契约由 `server.device.*` 与 `server.contacts.*` 场景覆盖，好友与群组由 `server.friends.http` 与 `server.friend_groups.http` 覆盖（两个设备各自持有 API Key，验证长有效期邀请码、重复加好友、非群主解散、群主退群与成员退群）。
...
非空 transcript 是 ASR transport qualification；随后仍继续执行独立归属的端到端发布
gate，要求 2 秒内出现 assistant text、3 秒内出现 assistant audio。transcript qualification
通过不能把红色的端到端结果判成通过。独立的 Push-to-Talk 与 Realtime terminal roundtrip
...
连续播放模拟按到包轨迹重放一个客户端：先缓冲 `prebuffer_ms`（500 ms）再按自身时钟
播放。`minimum_buffer_ms` 是整条流里缓冲量的最低点，为负表示某个空档超过了当时
已缓冲的音频，听感上就是卡顿；`underruns` 是欠载次数，`underrun_ms` 与
`max_underrun_ms` 是听众实际听到的静音总长与单次最长静音。P95 间隔无法反映单次
