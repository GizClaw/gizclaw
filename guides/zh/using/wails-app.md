...
Server、apply 内嵌 Credential/Tenant/Model/Workflow/RuntimeProfile/Gameplay catalog、同步
所需 Voice，并上传 PetDef assets。全部完成后详情自动切换为正常界面。
...
Workflow 不包含服务端 icon 或 i18n。Play 以 RuntimeProfile alias 作为 runtime Workflow ID，并由客户端本地映射展示名称与图标；owned Workflow 则显示其稳定名称。
