...
和每台 Server 对应的 Admin identity，不要求用户粘贴私钥。
...
Admin 和 Play identity 自动生成。添加远程 Server 后，背面显示对应 Admin
public key，用户把它写入目标 Server 的 `admin-public-key` 配置；private key
只保留在本机。Admin 和 Play 点击后在系统浏览器打开，而不会在 Wails 窗口内嵌
业务 UI。Pod 的操作菜单可编辑声明式配置、在系统文件管理器中显示目录，或确认
后删除 Pod。
