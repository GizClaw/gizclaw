...
本规范适用于 `sdk/js`、共享 JavaScript package、生成的 OpenAPI client，以及 Node 或 Playwright test harness。
...
- browser、Node 和 test runtime 对 global、storage、crypto、URL、stream 与 timer 的支持不同，使用前必须明确目标运行时。
...
- parser、转换、SDK helper 和错误处理使用 unit test；跨 package 或依赖真实 browser 行为时使用 integration test。
