 ...
 *
 * Upload raw bytes for a bin declared in the firmware channel configuration. The server stores the payload in the configured firmware asset object store and updates server-owned artifact metadata.
 */
export const uploadFirmwareBin = <ThrowOnError extends boolean = false>(options: Options<UploadFirmwareBinData, ThrowOnError>) => (options.client ?? client).put<UploadFirmwareBinResponses, UploadFirmwareBinErrors, ThrowOnError>({
    bodySerializer: null,
