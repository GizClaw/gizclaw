...
test("thinking parameter prefers the request parameter and falls back to the level parameter", () => {
  assert.equal(thinkingParameter({ thinking_param: "enable_thinking", thinking_level_param: "reasoning_effort" }), "enable_thinking");
  assert.equal(thinkingParameter({ thinking_level_param: "reasoning_effort" }), "reasoning_effort");
});
