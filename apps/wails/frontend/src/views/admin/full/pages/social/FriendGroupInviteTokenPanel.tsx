...
export function FriendGroupInviteTokenPanel({
  groupID,
}: FriendGroupInviteTokenPanelProps): JSX.Element {
  ...
  const load = async (): Promise<void> => {
    ...
    try {
      const current = await expectData(
        getFriendGroupInviteToken({ path: { id: groupID } }),
      );
    ...
    <Card>
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
        <div className="space-y-1">
          ...
          <CardDescription>
            Admin can directly set or clear the group invite token and
            expiration time.
          </CardDescription>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant={token.trim() === "" ? "outline" : "secondary"}>
            {token.trim() === "" ? "Absent" : "Present"}
          </Badge>
          <Button
        ...
        {loading ? (
          ...
          <>
            <div className="grid gap-4 xl:grid-cols-2">
              <FormField
                description="Leave empty only when you intend to clear the token."
                label="Invite token"
              >
                <Input
              ...
              <FormField
                description={`Current effective expiration: ${formatDate(expiresAt)}`}
                label="Expires at"
              >
                <DateTimeInput
                  disabled={busy !== ""}
            ...
            <div className="flex flex-wrap justify-end gap-2 border-t pt-4">
              ...
              <Button
                disabled={
                  busy !== "" || token.trim() === "" || expiresAt.trim() === ""
                }
                onClick={() => void save()}
                size="sm"
                type="button"
