...
type ProviderTenant =
  OpenAiTenant | GeminiTenant | DashScopeTenant | DeepSeekTenant;
...
type ProviderTenantConfig<T extends ProviderTenant> = {
  ...
  list(query: {
    cursor?: string;
    limit?: number;
  }): Promise<{ has_next: boolean; items: T[]; next_cursor?: string | null }>;
...
const openAIConfig: ProviderTenantConfig<OpenAiTenant> = {
  ...
  listDescription:
    "OpenAI-compatible runtime tenants used by model and voice provider bindings.",
  name: "OpenAI Tenant",
  resourceKind: "OpenAITenant",
  routeBase: "/providers/openai-tenants",
  summaryDescription:
    "OpenAI-compatible endpoint configuration and credential binding.",
...
const geminiConfig: ProviderTenantConfig<GeminiTenant> = {
  ...
  listDescription:
    "Gemini tenant records that bind provider credentials to Gemini project settings.",
  name: "Gemini Tenant",
...
const dashScopeConfig: ProviderTenantConfig<DashScopeTenant> = {
  ...
  listDescription:
    "DashScope tenant records that bind credentials to Aliyun-compatible provider endpoints.",
...
const deepSeekConfig: ProviderTenantConfig<DeepSeekTenant> = {
  ...
  listDescription:
    "DeepSeek tenant records that bind credentials to DeepSeek endpoints.",
...
function ProviderTenantsListPage<T extends ProviderTenant>({
  config,
}: {
  ...
  const {
    ...
    prevPage,
    refresh,
  } = useCursorListPage<T>(async (query) => {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {loading ? (
            ...
            <EmptyState
              description="Tenant records will appear here after they are created."
...
function ProviderTenantDetailPage<T extends ProviderTenant>({
  config,
}: {
  config: ProviderTenantConfig<T>;
}): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <EmptyState
          description={`This ${config.name} could not be loaded.`}
        ...
        <Tabs defaultValue="summary">
          ...
          <TabsContent className="space-y-4" value="summary">
            <DetailBlock
              items={config.detailFields(tenant)}
              title={config.name}
            />
          </TabsContent>
